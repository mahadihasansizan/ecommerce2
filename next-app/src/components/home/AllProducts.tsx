...
const AllProducts = ({ initialProducts }: AllProductsProps) => {
  const [products, setProducts] = useState<WooProduct[]>(initialProducts || []);
  const [loading, setLoading] = useState(!initialProducts);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1);
  const [hasMore, setHasMore] = useState(!initialProducts || initialProducts.length === 10);
    ...
    <>
      ...
      <section className=" py-4 bg-muted/20">
        <div className="container mx-auto px-1">
          ...
          {/* Products Grid */}
          {loading ? (
            <div className="all-products-grid mb-12">
